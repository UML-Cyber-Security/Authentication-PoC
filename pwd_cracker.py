...
subparsers = parser.add_subparsers(help='Run without any arguments for full help')

# create the parser for the "a" command
parser_a = subparsers.add_parser('hashFile', help='Crack a single file')
parser_a.add_argument('file', help='Leaked hash file to crack')
parser_a.add_argument('--is-salted', action="store_true", help='Salted hash file?')
parser_a.add_argument('rainbow', help="path to text file containing rainbow table names")
parser_a.add_argument('outputFile', help="where to write output")

# create the parser for the "b" command
parser_b = subparsers.add_parser('hashDir', help='Crack a directory of files')
parser_b.add_argument('leaked hash directory')
parser_b.add_argument('FILE_CONTAINING_RB-TBL-FL_NAMES')
parser_b.add_argument('output file', help="where to write output")
...
for subparsers_action in subparsers_actions:
    for choice, subparser in subparsers_action.choices.items():
        subhelp += f"{choice} "
        subhelp += subparser.format_help()
        subhelp += '-'*100+'\n'

args = parser.parse_args(args=None if sys.argv[1:] else print(subhelp))
...
def main():
    ...
    if len(sys.argv) == 4:
        start = time.time()
        solutions, status = crack_encrypted(groupEncryptedFiles(
            sys.argv[1]), rainbow_table_agrigator(sys.argv[2]))
    ...
    start = time.time()
    status = password_cracker(sys.argv[1], bool_maker(
        sys.argv[2]), rainbow_table_agrigator(sys.argv[3]), sys.argv[4])
    end = time.time()
    print(f'Time Elapsed: {time_formater(end - start)}                      ')
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    with open('timer.txt', 'a') as timer_file:
        timer_file.write(f'Hash-Crack took {time_formater(end - start)}'.ljust(
            48)+f' at {current_time} (type={salt_type(is_salted)}, status={status_formater(status)})\n')
...
def crack_unsalted(shadow_file: str, rainbowtable: str, output_file: str) -> bool:
    ...
    counter = 0
    print(
        f'cracked {counter} of {size}, {round(counter / size * 100,2)}% complete', end='\r')
...
def crack_encrypted(file_tuples: list, rainbow_tables: list):
    ...
    for i, (username, sltFile, encFile) in enumerate(file_tuples):
        ...
        if index == -1:
            ...
            failed += 1
            print(
                f'cracked {i - failed + 1}, failed {failed} of {size} aprox {round((i+1)/size*100,1)}% complete', end='\r')
            continue
        ...
        for iHash, hash in enumerate(rbLines):
            hash = (
                hash[2:-1]).encode().decode('unicode_escape').encode("raw_unicode_escape")
            try:
