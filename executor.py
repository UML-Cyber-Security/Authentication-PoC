...
description = "The executor.py file was built to let you manually add users"\
    "and check user-pswd combinations for validity"
...
def login():
    ...
    for i, line in enumerate(lines):
        ...
        if eval(hash) == hasher.calc_hash(eval(salt), password):
            ...
            print(
                f'  user {username} with password {safe_password} is the {num_format(i + 1)} user')
            return True
    return False
...
def listen():
    ...
    while True:
        ...
        if comand in leave_request:
            ...
            print(f'  There are {get_num_users()} users recorded in the shadow.txt file at the moment')
            continue
